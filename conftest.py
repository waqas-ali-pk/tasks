...
def test_create_admin_user1(django_user_model, test_user_email1, test_user_password1):
    user = django_user_model.objects.create(
        email=test_user_email1, username=test_user_email1
    )
...
def test_create_admin_user2(django_user_model, test_user_email2, test_user_password2):
    user = django_user_model.objects.create(
        email=test_user_email2, username=test_user_email2
    )
...
@pytest.fixture
def test_user_token_jwt1(
    api_client, test_create_admin_user1, test_user_email1, test_user_password1
):
    response = api_client.post(
        reverse("token_obtain_pair"),
        {"username": test_user_email1, "password": test_user_password1},
        format="json",
    )
...
@pytest.fixture
def test_user_token_jwt2(
    api_client, test_create_admin_user2, test_user_email2, test_user_password2
):
    response = api_client.post(
        reverse("token_obtain_pair"),
        {"username": test_user_email2, "password": test_user_password2},
        format="json",
    )
